...
# รวม 14 Core Modules แบบระบุชัดเจน
...
for module in pkgutil.iter_modules([str(base_path)]):
    try:
        mod = importlib.import_module(f"{module.name}.main")
        if hasattr(mod, "router"):
            app.include_router(mod.router, prefix=f"/{module.name}")
            print(f"Mounted: /{module.name}")
...
def root():
    return {
        "status": "ok",
        "message": "NaMo Framework API is running"
    }
